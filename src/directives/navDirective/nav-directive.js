import joeNavHtml from './joe-nav.html';
...
export default function() {
	return {
		...
		template: joeNavHtml,
		scope: {},
		controller: function($scope, $rootScope, underConstructionService, $window, navSrv) {
		...
		link: function(scope, el, attr) {
			...
			scope.$watch('stateName', function() {
				...
				if(name === 'skills' || name === 'devwork') {
					angular.element(document.querySelectorAll('.bar')).css('background-color', '#232323');
					...
					angular.element(document.querySelectorAll('.bar')).css('background-color', '#fff');
			...
			scope.toggleNav = function() {
				var display = angular.element(document.querySelectorAll('.nav-item')).css('display');
				if(display === 'none') {
					angular.element(document.querySelectorAll('.nav-item')).addClass('animated fadeIn');
					angular.element(document.querySelectorAll('.nav-item')).css('display', 'list-item');
					el.css('height', '100vh');
				}
				else {
					angular.element(document.querySelectorAll('.nav-item')).css('display', 'none');
